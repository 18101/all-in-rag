...
docker compose up：读取当前目录下 docker-compose.yml 配置文件，创建 / 启动里面定义的所有容器、网络、卷。

-d：--detach，后台守护进程模式，容器在后台运行，不占用终端，不会打印实时日志到控制台。

docker run：单容器命令行启动，一次只启动一个容器，所有参数都写在命令行。

docker compose up -d：多容器编排工具，用 yaml 文件描述一整套服务，一键启动多个关联容器。
...
- **HNSW (基于图的索引)** (Hierarchical Navigable Small World：分层可导航小世界图)
...
- **DiskANN (基于磁盘的索引)** (Disk‑based Approximate Nearest Neighbor：基于磁盘的近似最近邻搜索)
...
| RRFRanker | WeightedRanker 加权融合 |
| ---- | ---- |
| 只看排名位置，不看原始分数 | 使用原始相似度得分，需要归一化 |
| 不需要设置权重，自动平衡各方 | 需要手动设置各路权重（如 0.7+0.3） |
| 各路检索地位相对平等，适合不知道谁更重要 | 明确知道哪一路检索更重要时使用 |
| 适合：混合检索、多模型召回、多模态检索 | 适合：业务上明确优先级，关键词更重要 / 向量更重要 |
