...
这四种方法是向量数据库实现近似最近邻搜索（ANN，Approximate Nearest Neighbor）的核心技术索引路线。

因为在海量高维向量中，如果要一个个计算距离（暴力搜索 / Exact Search），计算量极高。这四种方法分别通过**空间切分、哈希映射、图拓扑结构**和**数据压缩**，牺牲微小的精度换取极高的搜索速度：

---

### 1. 基于树的方法（Tree-based）

> **代表算法**：Annoy（Spotify 开源）、KD-Tree、Random Projection Tree

* **原理**：
在向量空间中，随机选择超平面（Hyperplane）将所有向量分割成两半，然后对每个子空间继续用超平面切分，递归构建出一棵二叉树（随机投影树）。
* **如何搜索**：
当有一个查询向量进来时，只需从树根出发，根据查询向量在各个切分超平面的哪一侧，一路向下走（判断是去左子树还是右子树），最终快速到达包含候选向量的叶子节点。
* **特点**：
* **搜索复杂度下降**：从全局遍历的 $O(N)$ 降到树查找的对数复杂度 $O(\log N)$。
* **局限**：在高维空间（比如 512 维以上）中，数据会遭遇“维数灾难”，树的划分边界容易丢失近邻，因此常需构建多棵树（森林）组合搜索。



---

### 2. 基于哈希的方法（Hash-based）

> **代表算法**：LSH（Locality-Sensitive Hashing，局部敏感哈希）

* **原理**：
传统哈希函数（如 MD5）追求“敏感性”，即输入变动一点，哈希值天差地别；而 **局部敏感哈希（LSH）反其道而行之**，设计特殊的哈希函数，使得**空间中距离越近的向量，发生哈希碰撞（映射到同一个哈希桶/Bucket）的概率越高**。
* **如何搜索**：
检索时，将查询向量通过同样的哈希函数计算出桶编号，**只在对应的哈希桶以及邻近桶中去寻找最近邻**，避免遍历整库。
* **特点**：
* **快速过滤**：把复杂的连续高维距离计算转换为离散的桶查找。
* **局限**：对哈希函数的选择依赖极强，容易误判；在大规模高维数据集上召回率（Recall）不如基于图和量化的方法。
...
> **代表算法**：HNSW（Hierarchical Navigable Small World）、NSG

* **原理**：
把每个向量看作图中的一个节点，向量之间的相似度看作边的权重，构建一张“小世界网络”。
* **分层结构（HNSW 的精髓）**：类似于跳表（Skip List）或导航地图。顶层是稀疏的长距离高速公路（节点少、边长），底层是稠密的高速局部路网（节点多、边短）。


* **如何搜索**：
1. 从最顶层入口点出发，沿着能让距离变近的节点大步跳跃。
2. 局部最优后下降到下一层继续小步寻找。
3. 最终落到最底层（全量数据层），在极小区域内定位到精确的最近邻。


* **特点**：
* **性能极强**：是目前综合召回率（Recall）**和**检索速度（QPS）表现最好的算法之一，被 Milvus、Qdrant、Pinecone 等主流向量数据库广泛采用。
* **局限**：构建索引耗费内存（需要存储大量的图边关系），索引构建耗时较长。
...
> **代表算法**：Faiss 中的 IVF（倒排索引）与 PQ（乘积量化）

* **原理**：通过**压缩向量**和**缩小搜索范围**来提升效率。
1. **IVF（Inverted File，倒排索引/聚类）**：用 K-Means 算法把向量空间聚类成 $K$ 个簇（比如 1000 个簇心）。查询时，先判断查询向量属于哪几个簇心，**只在这几个簇对应的列表中查找**。
2. **PQ（Product Quantization，乘积量化）**：将一个高维向量拆成若干子向量（例如 128 维拆成 8 个 16 维的子段），对每个子段分别做聚类量化，用简短的编码（Code）代替原来的浮点数。


* **如何搜索**：
结合 IVF+PQ 时，先用 IVF 确定属于哪些簇，再用 PQ 的查表法（Lookup Table）快速计算近似距离，极大地减少 CPU 内存带宽占用和计算开销。
* **特点**：
* **极省内存**：能将内存占用压缩数倍甚至数十倍，使得单机存储和检索数亿级向量成为可能（如 Meta 的 Faiss）。
* **局限**：因为对向量进行了有损压缩，相比图方法会有微小的精度损失。
  
## 三、主流向量数据库介绍

![向量数据库分类图](./images/3_3_1.webp)

**Dedicated Vector Databases** 直译为 **“专用向量数据库”**（或“原生向量数据库”）。它是在向量数据库分类中与 **"Databases that support vector search"（支持向量检索的传统数据库）** 相对立的一个概念。

具体区别与含义如下：

#### 1. 核心含义：为向量而生的原生数据库

**Dedicated Vector Databases** 指的是**从架构设计的第 1 行代码开始，就是专门为了存储、索引和检索高维向量而构建的数据库系统**。

* **代表产品**：Milvus、Qdrant、Pinecone、Chroma、Weaviate、Vespa、LanceDB 等。
* **特点**：
* **底层优化**：底层存储和计算引擎专门针对 HNSW、IVF、PQ 等 ANN（近似最近邻）索引算法进行了优化，能极度高效地利用内存与 GPU/CPU 缓存。
* **高性能高并发**：在处理数十亿级海量高维向量检索时，延迟极低、吞吐量（QPS）极高。
* **AI 生态原生**：天生与 LangChain、LlamaIndex、PyTorch 等 AI 框架深度整合。



---

#### 2. 它的对立面：Databases that support vector search（扩展型/兼容型向量数据库）

这类数据库原本是传统的关系型数据库、搜索引擎或 Key-Value 数据库，后来**通过增加插件或扩展模块来支持向量功能**。

* **代表产品**：
* **PostgreSQL**（通过 `pgvector` 插件）
* **Elasticsearch / OpenSearch**（引入 k-NN 字段类型）
* **Redis**（通过 Redis Search 模块）
* **ClickHouse**、**Cassandra** 等


* **特点**：
* 适合在已有传统业务数据库的基础上快速引入向量检索，无需维护额外的新数据库。
* 但在极大规模向量（如亿级以上）或高并发实时检索场景下，由于其底层并非专为向量计算设计，性能和内存开销通常不如 Dedicated Vector Databases（专用向量数据库）。

---

当前主流的向量数据库产品包括：

[ **Pinecone** ](https://www.pinecone.io/)(/ˈpaɪn.koʊn/)是一款完全托管的向量数据库服务，采用Serverless架构设计（无服务器架构是一种云原生软件架构范式，其核心理念是让开发者专注于业务代码的编写，而将所有底层的服务器运维、容量规划、弹性扩缩容以及高可用管理完全交给云服务商处理）。它提供存储计算分离、自动扩展和负载均衡等企业级特性，并保证99.95%的SLA。Pinecone支持多种语言SDK，提供极高可用性和低延迟搜索（<100ms），特别适合企业级生产环境、高并发场景和大规模部署。

[ **Milvus** ](https://github.com/milvus-io/milvus)(/ˈmɪl.vəs/)是一款开源的分布式向量数据库，采用分布式架构设计，支持GPU加速和多种索引算法。它能够处理亿级向量检索，提供高性能GPU加速和完善的生态系统。Milvus特别适合大规模部署、高性能要求的场景，以及需要自定义开发的开源项目。

[ **Qdrant** ](https://github.com/qdrant/qdrant)(/ˈkwɒ.drənt/)是一款高性能的开源向量数据库，采用Rust开发，支持二进制量化技术。它提供多种索引策略和向量混合搜索功能，能够实现极高的性能（RPS>4000）和低延迟搜索。Qdrant特别适合性能敏感应用、高并发场景以及中小规模部署。

[ **Weaviate** ](https://github.com/weaviate/weaviate)(/ˈwiː.vi.eɪt/)是一款支持GraphQL的AI集成向量数据库，提供20+AI模块和多模态支持。它采用GraphQL API设计，支持RAG优化，特别适合AI开发、多模态处理和快速开发场景。Weaviate具有活跃的社区支持和易于集成的特点。

[ **Chroma** ](https://github.com/chroma-core/chroma)(/ˈkroʊ.mə/)是一款轻量级的开源向量数据库，采用本地优先设计，无依赖。它提供零配置安装、本地运行和低资源消耗等特性，特别适合原型开发、教育培训和小规模应用。Chroma的部署简单，适合快速原型开发。

[**FAISS**](https://github.com/facebookresearch/faiss)（Facebook AI Similarity Search）(/faɪs/ 或 /feɪs/) 是由 Meta（Facebook）AI 研究团队（FAIR）开源的高性能向量相似度检索与聚类库。
